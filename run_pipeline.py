...
def run_qpp_only(args, retriever, queries, relevance_lists):
    ...
    for name, model in models.items():
        ...
        logger.info("[%s] Pearson=%.4f | Spearman=%.4f | Kendall=%.4f",
...
def run_ablation(args, retriever, queries, relevance_lists):
    ...
    GROUPS = {
        ...
        "-- Similarity-based":               list(range(5, 12)),
    ...
    for label, keep_cols in GROUPS.items():
        ...
        logger.info("%-42s Pearson=%.4f | Spearman=%.4f | Kendall=%.4f",
                    label, corr["pearson_r"], corr["spearman_rho"], corr["kendall_tau"])
...
def main():
    ...
    if args.mode == "qpp_only":
        ...
        generator = build_generator(args.generator, hf_token=args.hf_token)
        ...
        # ── Save results -- includes lambda_used and gen_mode ─────────────
        ...
        with open(out, "w") as f:
            for r in results:
                f.write(json.dumps({
                    ...
                    "k_used":      r.k_used,
                    "lambda_used": round(r.lambda_used, 4),  # context-influence weight
                    "gen_mode":    r.gen_mode,               # RETRIEVAL_PRIMARY / BALANCED / KNOWLEDGE_PRIMARY
                    "answer":      r.answer,
                    "rouge_l":     r.rouge_l,
                    "bert_f1":     r.bert_f1,
                    "token_f1":    r.token_f1,
